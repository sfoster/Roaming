...
exports = module.exports = function directory(root, options){
  ...
  return function directory(req, res, next) {
    ...
    if (~path.indexOf('\0')) return next(handleError(400));

    // malicious path, forbidden
    if (0 != path.indexOf(root)) return next(handleError(403));
...
exports.html = function(req, res, files, next, dir, showUp, icons){
  fs.readFile(__dirname + '/../assets/directory.html', 'utf8', function(err, str){
    if (err) return next(err);
...
function load(icon) {
  ...
  return cache[icon] = fs.readFileSync(__dirname + '/../assets/icons/' + icon, 'base64');
}
