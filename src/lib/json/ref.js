      ...
      function serialize(it,path,_indentStr){
        if(typeof it == 'object' && it){
          ...
          if(it instanceof Date){ // properly serialize dates
            return '"' + date.toISOString(it,{zulu:true}) + '"';
          }
          var id = it.__id;
          ...
          var nextIndent = prettyPrint ? _indentStr + toJsonIndentStr : "";
          var newLine = prettyPrint ? "\n" : "";
          var sep = prettyPrint ? " " : "";

          if(it instanceof Array){
            var res = map(it, function(obj,i){
